...
def RXInput():
    global Round
    print("RX Input\n")
    PriveMode = input("Input Select Mode")
    if PriveMode == b'c':#模式选择：c-扫码传输颜色组
        GetCode()
    elif PriveMode == b'f': #模式选择：f-旧版圆盘识别
        Round=1
    ...
    elif PriveMode == b'r': #模式选择：r-第二次旧版圆盘识别，程序相同
        Round=2
        CameraStartup()
        CVDisk()
        uart.write(str('s'))
    elif PriveMode == b's': #模式选择：s-新版圆盘识别
        Round=1
        CameraStartup()
        CVDiskNew()
        uart.write(str('j'))
    elif PriveMode == b'b': #模式选择：b-第二次新版圆盘识别，程序相同
        Round=2
        CameraStartup()
        CVDiskNew()
        uart.write(str('s'))
    elif PriveMode == b'd': #模式选择：d-自由打靶(决赛)
        CameraStartup()
        CVShooting()
        uart.write(str('d'))
    elif PriveMode == b't': #模式选择：t-阈值显示器
        CameraStartup()
        ColorThresholds()
    elif PriveMode == b'i': #模式选择：i-圆阈值显示器
        CameraStartupHD()
        CircleThresholds()
    elif PriveMode == b'n': #模式选择：n-导航模式
        CameraStartupHD()
        Navigation()
