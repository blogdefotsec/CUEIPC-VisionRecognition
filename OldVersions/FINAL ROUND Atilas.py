...
###########################################
#               固件说明：                 #
#  相对于ALT固件，追加了第二区域识别的功能  #
###########################################
#               版本号说明：               #
#     0.1-仅有程序框架，需要更新roi         #
# 0.9-增加了数个通用型函数来方便后续二次开发 #
#            XYNOSURE 2023-12-03          #
###########################################
...
FTH01 = 96 #定位01位置X轴分界线
FTH12 = 186 #定位12位置X轴分界线
#-圆盘
roiFF=[114,101,68,71] #定位前一个靶的区域
roiBB=[114,101,68,71] #定位后一个靶的区域
roiLow=[70,129,188,109] #定位正位靶的位置
BlobStateFF = [0,0,0]
BlobStateBB = [0,0,0]
...
TargetGreen = (62, 71, -33, -10, 11, 20)#绿色靶阈值
...
def FinalSecondAreaDetect():
    ...
    while finishFlag==0:
        ...
        #获取颜色对象
        for i in range(3):
            BlobStateSAD1[i] = img.find_blobs(color_thresholds[i], pixels_threshold=SADpix, area_threshold=SADarea, merge=True, roi=roiSAD1)
            if BlobStateSAD1[i]:
                pass
            else:
                BlobStateSAD1[i] = [0,0,0]
        #识别结束后，判断是否识别到颜色
        for i in range(3):
    ...
    while finishFlag==0:
        img = sensor.snapshot().lens_corr(1.8)
        for i in range(3):
            BlobStateSAD2[i] = img.find_blobs(color_thresholds[i], pixels_threshold=SADpix, area_threshold=SADarea, merge=True, roi=roiSAD2)
            if BlobStateSAD2[i]:
                pass
            else:
                BlobStateSAD2[i] = [0,0,0]
        for i in range(3):
    ...
    while finishFlag==0:
        img = sensor.snapshot().lens_corr(1.8)
        for i in range(3):
            BlobStateSAD3[i] = img.find_blobs(color_thresholds[i], pixels_threshold=SADpix, area_threshold=SADarea, merge=True, roi=roiSAD3)
            if BlobStateSAD3[i]:
                pass
            else:
                BlobStateSAD3[i] = [0,0,0]
        for i in range(3):
...
#通过找圆的方式判断是否静止
def FinalIfStop():
    global CircleState, StopThreshold, roiStop
    CameraStartupHD()
    CircleState1 = 0
    CircleState2 = 0
    GetCircleState(roiStop)
    ...
    if CircleState1 == 0:
        ...
        print("No circle2?")
    elif abs(int(CircleState1.x())-int(CircleState2.x()))<=StopThreshold:
        print("FS-Move")
        stillflag = True
    else:
        print("FS-Still")
...
#通过找圆的方式判断是否移动
def FinalIfMove():
    global CircleState, MoveThreshold, roiStop
    CameraStartupHD()
    CircleState1 = 0
    CircleState2 = 0
    GetCircleState(roiStop)
    ...
    if CircleState1 == 0:
        ...
        print("No circle2?")
    elif abs(int(CircleState1.x())-int(CircleState2.x()))> MoveThreshold:
        print("FS-Move")
        stillflag = True
    else:
        print("FS-Still")
...
def FinalCVShooting():
    ...
    global Round,ColorCode1,ColorCode2,CVShootMode,BlobStateBB,BlobStateFF,BlobStateLow,BlobStateEmpty
    ColorReference = [0,0,0]
    WaitTime = 2 #等待时间
    PickTime = 2 #拿取时间
    PutTime = 3 #放置时间
    ...
    while True:
        ...
        GetBlobEmpty()
        flagempty = 0
        if BlobStateEmpty[0] == 0:
            flagempty = 1
        elif BlobStateEmpty[1] == 0:
            flagempty = 1
        elif BlobStateEmpty[2] == 0:
            flagempty = 1
        if flagempty == 0:
            break
    CameraStartup()
    ...
    GetBlobStateFF()
    GetBlobStateLow()
    #都获取，但根据CVShootMode选择检测前一个还是后一个。
    if CVShootMode == 1:
        CheckBlobState=BlobStateFF
    elif CVShootMode == 2:
        CheckBlobState=BlobStateBB
    elif CVShootMode == 3:
        CheckBlobState=BlobStateLow
    #如果指定的区域内的颜色刚好是目标颜色，则直接放置在1位置。
    for i in range(3):
        while True:
            if CheckBlobState[ColorReference[i]] != 0:
                time.sleep(WaitTime)
                uart.write(str(ColorReference[i]))
                time.sleep(PickTime)
...
while True:
    ...
    if (uart.any()):
        ...
        if ModeSelect == b'c':#模式选择：c-扫码传输颜色组
            ...
            CameraStartup()
            FinalCVCapture()
        elif ModeSelect == b'y': #模式选择：y-决赛打靶
            CameraStartup()
            FinalCVShooting()
